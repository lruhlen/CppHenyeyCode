...
void NewCalcCDEG(bundle &vars, vec2d &myG, vec3d &myC, vec3d &myD, vec3d &myE)
{
  ...
  for (int j = 0; j < jMax; j++)
    {
      ...
      // because that'll give you NANs in your matrices, and that's not good.
       ...
       // if (j+1 == jMax)
       // 	{ // Maybe I should be calling the atmos subroutine here, 
       // 	  // and using the Tatm, Patm, etc. values it generates to
       // 	  // calculate the dP/r/L/T values...?
      ...
      D[j][0][0] = (G1J(vars,j)  - G1[j]) / dP;
      D[j][1][0] = (G2J(vars,j)  - G2[j]) / dP;
      D[j][2][0] = (G3J(vars,j)  - G3[j]) / dP ;
      D[j][3][0] = (G4J(vars,j)  - G4[j]) / dP;
      if (j < jMax-1)
	{
	  C[j+1][0][0] = (G1J(vars,j+1)  - G1[j+1]) / dP;
	  C[j+1][1][0] = (G2J(vars,j+1)  - G2[j+1]) / dP;
	  C[j+1][2][0] = (G3J(vars,j+1)  - G3[j+1]) / dP;
	  C[j+1][3][0] = (G4J(vars,j+1)  - G4[j+1]) / dP;
	}
      if (j > 0)
	{
	  E[j-1][0][0] = (G1J(vars,j-1) - G1[j-1] ) / dP;
	  E[j-1][1][0] = (G2J(vars,j-1) - G2[j-1] ) / dP;
	  E[j-1][2][0] = (G3J(vars,j-1) - G3[j-1] ) / dP;
	  E[j-1][3][0] = (G4J(vars,j-1) - G4[j-1] ) / dP;
	}
      vars.P[j]-= dP;
      ...
      D[j][0][1] = (G1J(vars,j)  - G1[j]) / dr;
      D[j][1][1] = (G2J(vars,j)  - G2[j]) / dr;
      D[j][2][1] = (G3J(vars,j)  - G3[j]) / dr;
      D[j][3][1] = (G4J(vars,j)  - G4[j]) / dr;
      if (j < jMax-1)
	{
	  C[j+1][0][1] = (G1J(vars,j+1)  - G1[j+1]) / dr;
	  C[j+1][1][1] = (G2J(vars,j+1)  - G2[j+1]) / dr;
	  C[j+1][2][1] = (G3J(vars,j+1)  - G3[j+1]) / dr;
	  C[j+1][3][1] = (G4J(vars,j+1)  - G4[j+1]) / dr;
	}
      if (j > 0)
	{
	  E[j-1][0][1] = (G1J(vars,j-1)  - G1[j-1]) / dr;
	  E[j-1][1][1] = (G2J(vars,j-1)  - G2[j-1]) / dr;
	  E[j-1][2][1] = (G3J(vars,j-1)  - G3[j-1]) / dr;
	  E[j-1][3][1] = (G4J(vars,j-1)  - G4[j-1]) / dr;
	}
      vars.r[j]-= dr;
      ...
      D[j][0][2] = ( G1J(vars,j) - G1[j]) / dL;
      D[j][1][2] = ( G2J(vars,j) - G2[j]) / dL ;
      D[j][2][2] = ( G3J(vars,j) - G3[j]) / dL;
      D[j][3][2] = ( G4J(vars,j) - G4[j]) / dL;
      if (j < jMax-1)
	{
	  C[j+1][0][2] = ( G1J(vars,j+1)  - G1[j+1]) / dL;
	  C[j+1][1][2] = ( G2J(vars,j+1)  - G2[j+1]) / dL;
	  C[j+1][2][2] = ( G3J(vars,j+1)  - G3[j+1]) / dL;
	  C[j+1][3][2] = ( G4J(vars,j+1)  - G4[j+1]) / dL;
	}
      if (j > 0)
	{
	  E[j-1][0][2] = (G1J(vars,j-1)  - G1[j-1]) / dL;
	  E[j-1][1][2] = (G2J(vars,j-1)  - G2[j-1]) / dL;
	  E[j-1][2][2] = (G3J(vars,j-1)  - G3[j-1]) / dL;
	  E[j-1][3][2] = (G4J(vars,j-1)  - G4[j-1]) / dL;
	}
      vars.L[j]-= dL;
      ...
      D[j][0][3] = (G1J(vars,j) - G1[j] ) / dT;
      D[j][1][3] = (G2J(vars,j) - G2[j] ) / dT;
      D[j][2][3] = (G3J(vars,j) - G3[j] ) / dT;
      D[j][3][3] = (G4J(vars,j) - G4[j] ) / dT;
      if (j < jMax-1)
	{
	  C[j+1][0][3] = (G1J(vars,j+1) - G1[j+1]  ) / dT;
	  C[j+1][1][3] = (G2J(vars,j+1) - G2[j+1]  ) / dT;
	  C[j+1][2][3] = (G3J(vars,j+1) - G3[j+1]  ) / dT;
	  C[j+1][3][3] = (G4J(vars,j+1) - G4[j+1]  ) / dT;
	}
      if (j > 0)
	{
	  E[j-1][0][3] = (G1J(vars,j-1) - G1[j-1] ) / dT;
	  E[j-1][1][3] = (G2J(vars,j-1) - G2[j-1] ) / dT;
	  E[j-1][2][3] = (G3J(vars,j-1) - G3[j-1] ) / dT;
	  E[j-1][3][3] = (G4J(vars,j-1) - G4[j-1] ) / dT;
	}
      vars.T[j]-=dT;
      vars.eos_var_update(j);
    }
  ...
  // D[j][3][3] = -1.0;
   ...
   OneD atConstL(5), atConstR(5), originalAtm(5);
 ...
 D[j][1][1] = 1.0 - ( (RconstL - Rorig) / dr );
 D[j][1][2] = -1.0 * ( (RconstR - Rorig) / dL );
 D[j][3][1] =  ((TconstL - Torig) / dr) * -1.0;
 D[j][3][2] =  ((TconstR - Torig) / dL) * -1.0;

  // Testing-this-out form
  // D[j][0][1] = ((PconstL - Porig) / dr);
  // D[j][0][2] = ((PconstR - Porig) / dL);
  // D[j][1][1] =  ( (RconstL - Rorig) / dr ) - 1.0;
  // D[j][1][2] = ( (RconstR - Rorig) / dL );
  // D[j][3][1] =  ((TconstL - Torig) / dr) ;
  // D[j][3][2] =  ((TconstR - Torig) / dL) ;
